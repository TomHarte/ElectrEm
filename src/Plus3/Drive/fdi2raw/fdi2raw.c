/*
  ...
  More changes - eliminated zfile dependency, fixed various warnings.
  By Thomas Harte, 2006, thomas.harte@gmail.com
			...
			if (idx[i] >= maxidx) { /* stable pulse */
				...
				if (randval < (RAND_MAX / 2)) {
					if (randval > (RAND_MAX / 4)) {
						if (randval <= (3 * (Uint64)RAND_MAX / 8))
...
FDI *fdi2raw_header(gzFile f)
{
	...
	fdi->file = f;
	oldseek = gztell (fdi->file);
	gzseek (fdi->file, 0, SEEK_SET);
	gzread (fdi->file, fdi->header, 2048 );
	gzseek (fdi->file, oldseek, SEEK_SET);
...
int fdi2raw_loadtrack (FDI *fdi, uae_u16 *mfmbuf, uae_u16 *tracktiming, int track, int *tracklength, int *indexoffsetp, int *multirev, int mfm)
{
	...
	gzseek (fdi->file, fdi->track_offsets[track], SEEK_SET);
	gzread (fdi->file, fdi->track_src_buffer, fdi->track_src_len);
	memset (fdi->track_dst_buffer, 0, MAX_DST_BUFFER);
